            ...
            <div className="media-content">
                <div className="content">
                    {this.state.editing ? <UpdateComment postId={post_id} commentId={id} stopEdit={() => this.setState({ editing: false })} initialInput={comment_text} initialTags={tags.map(tag => tag.tag_name)} />
                        ...
                        <a className="has-text-primary"><span className="icon">{iLike ? <BomgSVG lit={true} scale={1.2} /> : <BomgSVG lit={false} scale={1.2} />}</span><span className="has-text-primary">{numLikes}</span></a> ·
